...
for video_id, shard_index in shard_dict.items():
    if video_id not in annotations.keys():
        continue
    ...
    with h5py.File(os.path.join(data_path, os.path.join('dino_s', h5_file_name)), 'r') as h5_file:
        ...
        new_metadata[video_id] = 0 # Use shard n0
...
with h5py.File('/scratch/project_465000977/data/YoutubeASL/subset/subset_YT.dino.train.0.h5', 'w') as subset_h5:
...
with open('/scratch/project_465000977/data/YoutubeASL/subset/subset_YT.annotations.dino.train.json', 'w') as f:
    json.dump(new_annotations, f, indent=4)
    print('Saving annotaitons')

# Save the new metadata JSON (shard indices)
with open('/scratch/project_465000977/data/YoutubeASL/subset/subset_YT.dino.train.json', 'w') as f:
